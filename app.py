...
def get_states_name(clientId: str):
    collection = db['states_collection']
    # Adjust the projection parameter to include only 'link' and '_id'
    documents = list(collection.find({'clientId': clientId}, {'link': 1, '_id': 1}))
    return documents


def generate_bot_json(clientId:str):
    bot_json = {}
    ...
    try:
        state_lists = get_states_name(clientId)

        for state_item in state_lists:
            bot_json.update(process_api_request(f'http://localhost:8000/get_state_flow_old/{state_item["_id"]}', 'disp_'))
...
@app.post("/download-bot-json")
def download_bot_json(flowNew: FlowNew):
    clientId = flowNew.clientId
    generate_bot_json(clientId = clientId)
    file_path = 'bot_flow.json'
...
async def save_state_flow(state:str,flow:Flow):
    collection = db['states_collection']
    filter = {'link': state, 'clientId': flow.clientId}
    print(filter)
    updated_flow = {"$set": {"nodes": flow.nodes, "edges": flow.edges}}
...
@app.post("/get_state_flow/{state}")
async def get_stat_flow(state:str,flowNew: FlowNew):
    collection = db['states_collection']
    try:
        filter = {'link':state, 'clientId': flowNew.clientId}
...
@app.get("/get_state_flow_old/{state}")
async def get_stat_flow_old(state:str,):
    collection = db['states_collection']
    try:
        filter = {'_id': ObjectId(state)}
        result = collection.find_one(filter=filter)
        if result:
            # Convert ObjectId to string (or you can simply not include it in the response)
            result['_id'] = str(result['_id'])
